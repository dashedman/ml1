...
**Байесовский классификатор** - класс алгоритмов, основаных на принципе выбора по максимальной апостериорной вероятности, тоесть по вероятности зависящей от условий. Является самым оптимальным аналитическим классификатором, если известны плотности распределения классов.

Формула определения класса(байесовское правило):
...
**Содержание:**
1. [Линии уровня](#линии-уровня-для-байесовского-классификатора)
2. [Наивный Байес](#наивный-байесовский-классификатор)
3. [Plug-in](#подстановочный-алгоритм-plug-in)
4. [Линейный Дискриминант Фишера](#линейный-дискриминант-фишера)
5. [Сеть радиальных базисных функций](#сеть-радиальных-базисных-функций)

## Линии уровня для байесовского классификатора
[В начало](#байесовский-классификатор)
...
`ND(x, u, Σ) = e^(-0.5 * (x - u)T * ~Σ * (x - u)) / sqrt((2 * pi)^k * |Σ|)`,

где **x** - наша искомая точка размерности **k**, **u** - точка центра распределения размерности **k**, **Σ** - ковариационная матрица, **~Σ** - обратная матрица, **(V)T** - транспонированый вектор **V**;
...
Если признаки некоррелированы и матрица диагональна, то линии уровня плотности распределения имеют форму эллипсоидов с центром **u** и осями, параллельными осям координат.

| u = (0, 0); Σ = [[2, 0], [0, 1]] | u = (0, 0); Σ = [[1, 0], [0, 3]] |
| -------------------------------- | -------------------------------- |
| ![Рис. 1.1](contour3_prev.png)   | ![Рис. 1.2](contour4_prev.png)   |
| [Интерактивный граффик](https://dashedman.github.io/contour3.html)  | [Интерактивный граффик](https://dashedman.github.io/contour4.html) |
...
![Рис. 1](contour5_prev.png)

[Интерактивный граффик](https://dashedman.github.io/contour5.html)
...
**Наивный байес.** Пусть, **n** общих признаков, которые мы определили выше, независимы и статичны. Тогда, используя свойство умножения вероятностей независимых событий, легко получить нашу функцию правдоподобия для всего класса:
...
Первый класс, растянут и разввернут к центру, обхватывая своей функцией правдоподобия больше пространства, второй класс расширяется вдоль оси Y.
...
Простой грубый алгоритм. Заключается в том что, мы сначала оцениваем мат. ожидание и дисперсию для выборки каждого класса, а затем просто подставляем эти оценки в байесовский классификатор.
...
Тогда разделяющая поверхность вырождается в линейную.
...
`Σ = 1/l * sum(i){(x[i] - μ_yi)*t(x[i] - μ_yi)}`,
